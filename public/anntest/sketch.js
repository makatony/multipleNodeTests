...
function setup() {
	...
	createP('N/A').id('neuronValue');
	button = createButton('Reset Visualization').mousePressed(function () { allNeuronsInOriginalPosition = false; });
	...
	for (var i = 0; i < network.layers.length; i++) {
		...
		for (var j = 0; j < thisLayer.neurons.length; j++) {
			...
			thisNeuron.viz.intersectsMouse = function () { return (this.pos.dist(createVector(mouseX, mouseY)) < NeuronRadius); } //here it is without .viz because the function is inside .viz already
...
function draw() {
	...
	if (!allNeuronsInOriginalPosition) moveToOriginalPos();
	...
	for (var i = 0; i < network.layers.length; i++) {
		...
		for (var j = 0; j < thisLayer.neurons.length; j++) {
			...
			if (thisNeuron.viz.intersectsMouse()) {	fill(255); focusNeuron = thisNeuron; select('#neuronValue').html(thisNeuron.value); }
			if ((!thisNeuron.viz.intersectsMouse()) && (focusNeuron == thisNeuron)) { focusNeuron = ""; }
			...
			for (var k = 0; k < network.layers[i-1].neurons.length; k++) {
				...
				if ((focusNeuron instanceof Neuron) && ((focusNeuron != thisPrevNeuron) && (focusNeuron != thisNeuron))) continue;
...
function moveToOriginalPos() {
	...
	for (var i = 0; i < network.layers.length; i++) {
		...
		for (var j = 0; j < thisLayer.neurons.length; j++) {
			...
			if (thisNeuron.viz.originalPos.dist(thisNeuron.viz.pos) < 5) thisNeuron.viz.pos = thisNeuron.viz.originalPos.copy();
